...
# 발주 관리 페이지
    ...
    # ========== 탭 3: 발주 추천 ==========
    # 주의: 탭 3은 매우 복잡하므로(800줄) 일단 app.py에 그대로 유지
    # 향후 세부 함수로 분리 예정
    with tab3:
            ...
            if not daily_sales_df.empty and not recipe_df.empty and not usage_df.empty:
                ...
                if not usage_df.empty:
                    ...
                    if not order_df.empty:
                        ...
                        # 발주 생성 기능은 일시 비활성화
                        render_section_divider()
                        st.info("📝 현재 버전에서는 자동 발주 생성 기능을 일시적으로 사용하지 않습니다.\n발주 추천 리스트와 CSV 다운로드만 활용해주세요.")
                    else:
    ...
    # ========== 탭 4: 발주 관리 (진행 현황) ==========
    with tab4:
        render_order_history_tab(ingredient_df, load_csv)
    
    # ========== 탭 5: 공급업체 ==========
    with tab5:
        render_supplier_management_tab(ingredient_df, load_csv)
...
# 통합 대시보드 페이지
    ...
    if breakeven_sales is not None and breakeven_sales > 0:
        ...
        run_abc = st.button("📊 이번 달 ABC 분석 실행", key="run_monthly_abc")
