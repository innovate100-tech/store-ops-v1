...
with st.sidebar:
    ...
    # 사이드바 네비게이션 - 카테고리별 구분 (IA 재정렬: 홈/입력/분석/설계/운영)
    ...
    menu_categories = {
        ...
        "✍ 입력 (빠른 입력)": [  # expander 내부 항목들
            ("일일 마감 입력", "일일 입력(통합)"),  # 라벨 변경: 일일 마감 → 일일 마감 입력
            ("QSC 입력", "건강검진 실시"),  # 라벨 변경: 매장 체크리스트 → QSC 입력
            ("월간 정산 입력", "실제정산"),  # 라벨 변경: 월간 정산 → 월간 정산 입력
            ("목표 매출 구조 입력", "목표 매출구조"),  # page key 유지
            ("목표 비용 구조 입력", "목표 비용구조"),  # page key 유지
        ...
        "🧠 설계": [
            ...
            ("수익 구조 설계", "수익 구조 설계실"),  # 라벨 변경, page key 유지
            ("레시피 설계", "레시피 등록"),  # 라벨 변경, page key 유지
            ("목표 비용 구조 입력", "목표 비용구조"),  # 라벨 변경, page key 유지
            ("목표 매출 구조 입력", "목표 매출구조"),  # 라벨 변경, page key 유지
        ],
        "🛠 운영": [
            ("직원 연락망", "직원 연락망"),
            ("협력사 연락망", "협력사 연락망"),
    ...
    for category_name in category_order:
        ...
        items = menu_categories[category_name]
        if not items:
            continue
...
# 통합 대시보드 페이지 (퇴역 - fail-safe stub)
    ...
    <div style="padding: 2rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                border-radius: 12px; color: white; margin: 2rem 0; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <div style="display: flex; align-items: center; margin-bottom: 1rem;">
            <span style="font-size: 2rem; margin-right: 1rem;">📊</span>
            <h2 style="color: white; margin: 0; font-size: 1.5rem; font-weight: 600;">통합 대시보드는 종료되었습니다</h2>
        </div>
        <div style="font-size: 1.1rem; line-height: 1.6; color: #f0f0f0; margin-top: 1rem;">
            분석 카테고리는 "매출 분석 / 판매·메뉴 분석 / 원가 분석" 3개로 재편되었습니다.<br>
            아래 버튼을 클릭하여 각 분석 페이지로 이동하세요.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        if st.button("💰 매출 분석", type="primary", use_container_width=True, key="redirect_sales"):
            st.session_state["current_page"] = "매출 관리"
            st.rerun()
    with col2:
        if st.button("📦 판매·메뉴 분석", type="primary", use_container_width=True, key="redirect_sales_volume"):
            st.session_state["current_page"] = "판매 관리"
            st.rerun()
    with col3:
        if st.button("💵 원가 분석", type="primary", use_container_width=True, key="redirect_cost"):
            st.session_state["current_page"] = "원가 파악"
            st.rerun()
...
# 통합 대시보드 페이지 (기존 코드는 archive/ui_pages_dashboard/ 참조)
# 라인 2344-3222 사이의 약 880줄 코드가 제거되었습니다.