...
# 통합 대시보드 페이지
    ...
    actual_settlement_df = load_csv('actual_settlement.csv', default_columns=[
        '연도', '월', '실제매출', '실제비용', '실제이익', '실제이익률'
    ])
    current_profit_rate = None
    if not actual_settlement_df.empty:
        current_settlement = actual_settlement_df[
            (actual_settlement_df['연도'] == current_year) & 
            (actual_settlement_df['월'] == current_month)
        ]
        if not current_settlement.empty:
            current_profit_rate = float(current_settlement.iloc[0].get('실제이익률', 0))
    ...
    with col1:
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
            ...
            <div style="color: white; font-size: 0.85rem; opacity: 0.8;">
                {status}
            </div>
        </div>
        """.format(
            breakeven=f"{breakeven_sales:,.0f}원" if breakeven_sales else "계산 불가",
            status=f"달성률 {period_sales/breakeven_sales*100:.1f}%" if breakeven_sales and period_sales > 0 else "데이터 필요"
        ), unsafe_allow_html=True)
    
    with col2:
        # 목표 매출 타일
        achievement_rate = (period_sales / target_sales * 100) if target_sales > 0 else 0
        status_color = "#4ade80" if achievement_rate >= 100 else "#fbbf24" if achievement_rate >= 80 else "#f87171"
        st.markdown("""
        <div style="background: linear-gradient(135deg, {color1} 0%, {color2} 100%); 
            ...
            <div style="color: white; font-size: 0.85rem; opacity: 0.8;">
                현재: {current}원 ({rate:.1f}%)
            </div>
        </div>
        """.format(
            color1=status_color,
            color2=status_color,
            target=f"{target_sales:,.0f}원" if target_sales > 0 else "미설정",
            current=f"{period_sales:,.0f}",
            rate=achievement_rate
        ), unsafe_allow_html=True)
    
    with col3:
        # 이익률 타일
        profit_color = "#4ade80" if current_profit_rate and current_profit_rate >= 20 else "#fbbf24" if current_profit_rate and current_profit_rate >= 10 else "#f87171"
        st.markdown("""
        <div style="background: linear-gradient(135deg, {color1} 0%, {color2} 100%); 
                    padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
                    margin-bottom: 1rem;">
        ...
        """.format(
            color1=profit_color,
            color2=profit_color,
            rate=f"{current_profit_rate:.1f}%" if current_profit_rate is not None else "데이터 없음",
            status="이번달 기준" if current_profit_rate is not None else "실제정산 필요"
        ), unsafe_allow_html=True)
    ...
    with col1:
        ...
        change_pct = (change / yesterday_sales * 100) if yesterday_sales > 0 else 0
        change_icon = "📈" if change >= 0 else "📉"
        st.markdown("""
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); 
                    padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
                    margin-bottom: 1rem; cursor: pointer;" onclick="window.location='?page=매출 관리'">
            <div style="color: white; font-size: 0.9rem; margin-bottom: 0.5rem; opacity: 0.9;">
                💵 오늘의 매출
            </div>
            <div style="color: white; font-size: 2rem; font-weight: 700; margin-bottom: 0.5rem;">
            ...
            <div style="color: white; font-size: 0.85rem; opacity: 0.8;">
                {icon} 어제 대비 {change:+,.0f}원 ({change_pct:+.1f}%)
            </div>
        </div>
        """.format(
            sales=today_sales,
    ...
    # 원가율 경고 메뉴
    high_cost_menus = []
    if not recipe_df.empty and not ingredient_df.empty and not menu_df.empty:
        cost_df = calculate_menu_cost(menu_df, recipe_df, ingredient_df)
        for _, row in cost_df.iterrows():
            cost_rate = (row['원가'] / row['판매가'] * 100) if row['판매가'] > 0 else 0
            if cost_rate >= 35:
                high_cost_menus.append({
                    '메뉴명': row['메뉴명'],
                    '원가율': cost_rate
                })
        high_cost_menus = sorted(high_cost_menus, key=lambda x: x['원가율'], reverse=True)
    ...
    with col3:
        ...
        if order_needed:
            order_html = f"""
            <div style="color: white; font-size: 1.2rem; font-weight: 700; margin-bottom: 0.5rem;">
                {len(order_needed)}개 재료
            </div>
            <div style="color: white; font-size: 0.9rem; opacity: 0.9;">
                예상 금액: {total_order_amount:,.0f}원
            </div>
            """
        else:
    ...
    # 현재 근무 직원 (session_state에서)
    staff_list = []
    if 'employees' in st.session_state and st.session_state.employees:
        # 오늘 근무 중인 직원 (간단히 모든 직원 표시, 실제로는 근무 스케줄 필요)
        for emp in st.session_state.employees[:5]:  # 최대 5명만 표시
            staff_list.append({
                '이름': emp.get('name', ''),
                '역할': emp.get('role', ''),
                '연락처': emp.get('phone', '')
    ...
    if not recipe_df.empty:
        recipe_count = len(recipe_df['메뉴명'].unique())
    recipe_rate = (recipe_count / menu_count * 100) if menu_count > 0 else 0
    
    # 최근 업데이트 시간
    last_sales_update = "없음"
    if not sales_df.empty:
        last_sales_date = sales_df['날짜'].max()
        last_sales_update = last_sales_date.strftime('%Y-%m-%d') if pd.notna(last_sales_date) else "없음"
    ...
    with col1:
        ...
        if staff_list:
            for staff in staff_list[:3]:
                ...
                """
            if len(staff_list) > 3:
                staff_html += f"<div style='color: white; font-size: 0.85rem; opacity: 0.7;'>외 {len(staff_list)-3}명</div>"
        else:
            staff_html = "<div style='color: white; font-size: 0.9rem; opacity: 0.7;'>등록된 직원 없음</div>"
        
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); 
                    padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
                    margin-bottom: 1rem; cursor: pointer;" onclick="window.location='?page=직원 연락망'">
            <div style="color: white; font-size: 0.9rem; margin-bottom: 0.5rem; opacity: 0.9;">
                👥 현재 근무 직원
            </div>
            <div style="color: white; font-size: 1.2rem; font-weight: 700; margin-bottom: 0.5rem;">
    ...
    with col2:
        ...
        <div style="background: linear-gradient(135deg, #14b8a6 0%, #0d9488 100%); 
                    padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);
                    margin-bottom: 1rem; cursor: pointer;" onclick="window.location='?page=레시피 등록'">
            <div style="color: white; font-size: 0.9rem; margin-bottom: 0.5rem; opacity: 0.9;">
                📝 레시피 등록 현황
            </div>
            <div style="color: white; font-size: 1.2rem; font-weight: 700; margin-bottom: 0.5rem;">
            ...
            <div style="color: white; font-size: 0.9rem; opacity: 0.8;">
                {recipe_count}개 / {menu_count}개 메뉴
            </div>
            <div style="color: white; font-size: 0.85rem; opacity: 0.7; margin-top: 0.3rem;">
                미등록: {menu_count - recipe_count}개
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div style="background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); 
            ...
            <div style="color: white; font-size: 0.9rem; margin-bottom: 0.3rem;">
                매출: {last_sales_update}
            </div>
            <div style="color: white; font-size: 0.85rem; opacity: 0.7;">
                실시간 동기화
            </div>
        </div>
        """, unsafe_allow_html=True)
