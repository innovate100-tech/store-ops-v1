"""
홈 문제/잘한 점 룰 (룰 기반)
- get_problems_top1, get_good_points_top1: 경량 버전 (TOP1만)
- get_problems_top3, get_good_points_top3: 전체 버전 (TOP3)
"""
...
def get_problems_top1(store_id: str) -> list:
    """
    문제 TOP1 추출 (경량 버전, 룰 기반)
    Returns: [{"text": str, "target_page": str}] 최대 1개
    """
    try:
        ...
        if len(sales_data) >= 6:
            ...
            if r3 and p3:
                ...
                pa = sum(p3) / len(p3)
                if ra < pa and pa > 0:
                    return [{"text": "최근 3일 평균 매출이 직전 기간보다 감소했습니다.", "target_page": "매출 관리"}]
        ...
        while d < today and d < end_m:
            ...
            d += timedelta(days=1)
        if gap:
            return [{"text": "이번 달 마감하지 않은 날이 있습니다.", "target_page": "점장 마감"}]
        return [{"text": "아직 분석할 데이터가 충분하지 않습니다.", "target_page": "점장 마감"}]
    except Exception:
        return [{"text": "문제 분석 중 오류가 발생했습니다.", "target_page": "점장 마감"}]


def get_good_points_top1(store_id: str) -> list:
    """
    잘한 점 TOP1 추출 (경량 버전, 룰 기반)
    Returns: [{"text": str, "target_page": str}] 최대 1개
    """
    try:
        supabase = get_supabase_client()
        ...
        if len(sales_data) >= 6:
            ...
            if r3 and p3 and sum(p3) / len(p3) > 0 and sum(r3) / len(r3) > sum(p3) / len(p3):
                return [{"text": "최근 3일 평균 매출이 이전 기간보다 증가했습니다.", "target_page": "매출 관리"}]
        cy, cm = today.year, today.month
        close_stats = get_monthly_close_stats(store_id, cy, cm)
        if close_stats[3] >= 3:
            return [{"text": "연속 마감 기록이 유지되고 있습니다.", "target_page": "점장 마감"}]
        return [{"text": "데이터가 쌓이면 자동 분석됩니다.", "target_page": "점장 마감"}]
    except Exception:
