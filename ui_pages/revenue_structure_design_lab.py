...
def _render_revenue_structure_design_tools(store_id: str, year: int, month: int, fixed_costs: float, variable_ratio: float, break_even: float):
    ...
    if not expense_df.empty:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📋 목표 비용 구조(입력)로 이동", key="goto_cost_structure", use_container_width=True, type="primary"):
                st.session_state["current_page"] = "목표 비용구조"
                st.rerun()
        with col2:
            if st.button("📊 목표 매출 구조로 이동", key="goto_sales_structure", use_container_width=True):
                st.session_state["current_page"] = "목표 매출구조"
                st.rerun()
    else:
        st.warning("비용 구조 데이터가 없습니다. 목표 비용 구조를 먼저 입력하세요.")
        if st.button("📋 목표 비용 구조(입력)로 이동", key="goto_cost_structure_empty", use_container_width=True, type="primary"):
            st.session_state["current_page"] = "목표 비용구조"
            st.rerun()
